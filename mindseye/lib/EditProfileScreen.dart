...
import 'package:mindseye/constants/roles.dart';
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void initState() {
    ...
    _clinicNameController =
        TextEditingController(text: widget.userData['clinicName'] ?? '');
  }

  // Map frontend roles to backend roles
  String _mapFrontendToBackendRole(String frontendRole) {
    return RoleMapping.normalizeRole(frontendRole);
  }
  ...
  Future<void> _saveProfile() async {
    ...
    final url = Uri.parse('$backendUrl/api/users/edit-profile');
    
    // Map frontend roles to backend roles
    final frontendRole = widget.userData['role']!;
    final backendRole = _mapFrontendToBackendRole(frontendRole);
    ...
    // Send newPhone for roles that change their phone field
    if (_phoneController.text.trim().isNotEmpty) {
      body['newPhone'] = _phoneController.text.trim();
    }
          ...
          child: Form(
            ...
            child: ListView(
              children: [
                ...
                // Phone Field (for Professionals/School Admins/Organization Admins only)
                if (role == Roles.professional || role == Roles.admin || role == Roles.organizationAdmin)
                  _buildInputField(
                      "Phone Number", Icons.phone, _phoneController,
                      keyboardType: TextInputType.number,
                      enabled: role != Roles.parent),
                ...
                // Clinic Name Field (only for Professionals)
                if (role == Roles.professional)
                  _buildInputField("Clinic Name", Icons.business, _clinicNameController),
