    ...
    try {
      ...
      if (isValid) {
        ...
        if (detectedRole == "Professional") {
          ...
          organizationIdToSave = profileData['organizationId']?.toString();
          // You could also save assignedSchoolIds if needed immediately
        } else if (detectedRole == "Parent") {
          nameToSave = profileData['name'];
          organizationIdToSave = profileData['organizationId']?.toString();
          // Parent org can be resolved via child/school later if needed
        } else if (detectedRole == "Teacher") {
          nameToSave = profileData['name'];
          organizationIdToSave = profileData['organizationId']?.toString();
        } else if (detectedRole == "Admin" || detectedRole == "NGO Master" || detectedRole == "OrganizationAdmin") {
          nameToSave = profileData['name'];
          organizationIdToSave = profileData['organizationId']?.toString();
        }
        ...
        print(
            "\u{1F4BE} Saved user details: Role=$detectedRole, Phone=${widget.mobileNumber}, Name=$nameToSave, Org=$organizationIdToSave");
