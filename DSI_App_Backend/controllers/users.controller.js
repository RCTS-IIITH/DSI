...
const getTeachersBySchool = async (req, res) => {
  try {
    ...
    // Teacher model stores school_name as string
    const teachers = await Teacher.find({ school_name: school.schoolName });
...
const editUserProfile = async (req, res) => {
  try {
    ...
    if (role === 'Parent') {
      ...
      const parent = await Child.findOne({ parentPhoneNumber: phoneInt });
      ...
      if (updateData.parentPhoneNumber) {
        const newPhone = parseInt(updateData.parentPhoneNumber, 10);
        if (!Number.isNaN(newPhone)) parent.parentPhoneNumber = newPhone;
      }
      await parent.save();
    ...
    if (role === 'SchoolAdmin' || role === 'NGOAdmin') {
      ...
      applyUpdates(admin, ['number', 'assignedSchoolList', 'role']);
      if (updateData.newPhone) {
  ...
  try {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      const child = new Child({
        ...
        parentPhoneNumber: parentPhoneNumber ? parseInt(parentPhoneNumber) : null,
        class: classInt,
        age: ageInt,
      });
