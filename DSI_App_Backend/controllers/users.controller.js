  ...
  try {
    ...
    if (!data.name || !data.Number || !data.Address || !data.ProfessionalID || !data.workEmail) {
      return validationErrorResponse(res, [
        ...
        { field: 'ProfessionalID', message: 'Professional ID is required' },
        { field: 'workEmail', message: 'Work email is required' }
      ]);
    }

    // Use organizationId from request or from middleware context
    const targetOrgId = data.organizationId || req.organizationId;
    ...
    if (existing) {
      return errorResponse(res, "ProfessionalID already exists", 400);
    }
    ...
    return standardResponse(res, true, "Professional account created successfully", { professionalId: professional._id }, 201);
  } catch (error) {
    console.log(error);
    return errorResponse(res, "Error Creating Professional Account", 500);
  }
};
...
const searchNumber = async (req, res) => {
  ...
  try {
    ...
    if (usertype == "Parent") {
      const parent = await Child.findOne({ parentPhoneNumber: number }); // Now consistently string
      if (!parent) {
...
const detectRoleAndFetchProfile = async (req, res) => {
  try {
    ...
    console.log(`🔍 Detecting role for phone: ${phoneNumber}, specified role: ${role || 'Any'}`);

    const result = await authenticateUser(phoneNumber, role);
    
    if (result.success) {
      return standardResponse(res, true, result.message, {
    ...
    console.error("Error in detectRoleAndFetchProfile:", error);
    return errorResponse(res, "Internal server error during role detection", 500);
  }
};
